...
class OMWBasicnessAnalyzer:
    def __init__(self, input_df: pd.DataFrame, basicness_weights: dict = None, opt_set_path: str = None, language_specific_weights: Dict[str, dict] = None):
        ...
        with open(get_path_from_root("resources/culture/culture_dict_lang_ilis.json"), "r") as f:
        ...
        if opt_set_path is not None:
            ...
            shuffled = self.__opt_set_df.sample(frac=1, random_state=42).reset_index(drop=True)
    ...
    def load_correlation_weights(correlation_csv_path: str) -> Dict[str, dict]:
        ...
        for lang in corr_df.columns:
            weights = {
                ...
                'pronounce_complexity_weight': corr_df.loc['avg_pronounce_complexity', lang],
                ...
                'word_in_children_res_weight': corr_df.loc['word_in_children_res', lang],
                'word_in_second_lang_learn_res_weight': corr_df.loc['word_in_second_lang_learn_res', lang],
    ...
    def __calculate_basicness_score(self, row: pd.DataFrame) -> float:
        ...
        if self.__language_specific_weights and row['Language'] in self.__language_specific_weights:
        ...
        word_in_second_lang_learn_res_weight = weights['word_in_second_lang_learn_res_weight']
        ...
        length_factor = (1 - normalized_length) if weights['word_length_weight'] < 0 else normalized_length
        pronounce_complexity_factor = (1 - normalized_pronounce_complexity) if weights['pronounce_complexity_weight'] < 0 else normalized_pronounce_complexity
        n_synonyms_factor = (1 - normalized_n_synonyms) if weights['n_synonyms_weight'] < 0 else normalized_n_synonyms
        n_senses_factor = (1 - normalized_n_senses) if weights['n_senses_weight'] < 0 else normalized_n_senses
        n_syn_senses_factor = (1 - n_syn_senses) if weights['n_syn_senses_weight'] < 0 else n_syn_senses
        word_in_children_res_factor = (1 - word_in_children_res) if weights['word_in_children_res_weight'] < 0 else word_in_children_res
        word_in_second_lang_learn_res_factor = (1 - word_in_second_lang_learn_res) if weights['word_in_second_lang_learn_res_weight'] < 0 else word_in_second_lang_learn_res

        dependent_terms = np.pow(normalized_frequency, abs(word_frequency_weight)) * \
                          np.pow(length_factor, abs(word_length_weight)) * \
                          np.pow(pronounce_complexity_factor, abs(pronounce_complexity_weight))

        independent_terms = abs(n_senses_weight) * n_senses_factor + \
                            ...
                            abs(word_in_second_lang_learn_res_weight) * word_in_second_lang_learn_res_factor + \
    ...
    def __calculate_basicness_score_softmax(self, row: pd.DataFrame) -> float:
        # Get language-specific weights if available, otherwise use default weights
        if self.__language_specific_weights and row['Language'] in self.__language_specific_weights:
            weights = self.__language_specific_weights[row['Language']]
        else:
        ...
        # Boolean features (already properly scaled 0-1)
        word_in_children_res = row['word_in_children_res']
        word_in_second_lang_learn_res = row['word_in_second_lang_learn_res']

        # Get normalized features instead of softmax
        ...
        normalized_n_synonyms = row['normalized_n_synonyms']
        normalized_n_senses = row['normalized_n_senses']
        n_syn_senses = row['n_syn_senses']  # Assuming this is already normalized
        
        # Handle signs based on correlation direction
        length_factor = word_length_weight * (1 - normalized_length if word_length_weight < 0 else normalized_length)
        pronounce_complexity_factor = pronounce_complexity_weight * (1 - normalized_pronounce_complexity if pronounce_complexity_weight < 0 else normalized_pronounce_complexity)
        n_synonyms_factor = n_synonyms_weight * (1 - normalized_n_synonyms if n_synonyms_weight < 0 else normalized_n_synonyms)
        n_senses_factor = n_senses_weight * (1 - normalized_n_senses if n_senses_weight < 0 else normalized_n_senses)
        n_syn_senses_factor = n_syn_senses_weight * (1 - n_syn_senses if n_syn_senses_weight < 0 and n_syn_senses <= 1 else n_syn_senses)
        word_in_children_res_factor = word_in_children_res_weight * (1 - word_in_children_res if word_in_children_res_weight < 0 else word_in_children_res)
        word_in_second_lang_learn_res_factor = word_in_second_lang_learn_res_weight * (1 - word_in_second_lang_learn_res if word_in_second_lang_learn_res_weight < 0 else word_in_second_lang_learn_res)

        # Using a linear weighted combination for more predictable behavior
        basicness_score = (
            word_frequency_weight * normalized_frequency +
            length_factor +
            pronounce_complexity_factor +
            n_hyponyms_weight * normalized_n_hyponyms +
            n_synonyms_factor +
            n_senses_factor +
        ...
        total_weight = (abs(word_frequency_weight) + abs(word_length_weight) + abs(pronounce_complexity_weight) +
                        abs(n_hyponyms_weight) + abs(n_synonyms_weight) + abs(n_senses_weight) +
                        abs(word_in_children_res_weight) + abs(word_in_second_lang_learn_res_weight) +
                        abs(n_syn_senses_weight))
                        
        if total_weight > 0:  # Avoid division by zero
            basicness_score = basicness_score / total_weight
        
        return basicness_score

    def analyze_lang_syn_group(self, word: str = None, thresholds: List[float] = None) -> pd.DataFrame:
        ...
        plain_metrics = df.groupby(['Language', 'ili']).agg(
        ...
        min_max_metrics_by_lang = plain_metrics.groupby('Language').agg(
        ...
        normalized['normalized_length'] = (normalized['avg_word_length'] - normalized['min_word_length']) / \
                                          (normalized['max_word_length'] - normalized['min_word_length'] + norm_length_denom)

        normalized['normalized_frequency'] = (normalized['avg_word_frequency'] - normalized['min_word_frequency']) / \
                                             (normalized['max_word_frequency'] - normalized['min_word_frequency'] + norm_freq_denom)

        normalized['normalized_pronounce_complexity'] = normalized['avg_pronounce_complexity']

        normalized['normalized_n_hyponyms'] = (normalized['n_hyponyms'] - normalized['min_n_hyponyms']) / \
                                              (normalized['max_n_hyponyms'] - normalized['min_n_hyponyms'] + n_hyponyms_denom)

        normalized['normalized_n_synonyms'] = (normalized['n_synonyms'] - normalized['min_n_synonyms']) / \
                                              (normalized['max_n_synonyms'] - normalized['min_n_synonyms'] + n_synonyms_denom)

        normalized['normalized_n_senses'] = (normalized['avg_n_senses'] - normalized['min_n_senses']) / \
                                            (normalized['max_n_senses'] - normalized['min_n_senses'] + n_senses_denom)

        normalized['softmax_length'] = np.exp(normalized['avg_word_length']) / np.sum(np.exp(normalized['avg_word_length']))
        normalized['softmax_frequency'] = np.exp(normalized['avg_word_frequency']) / np.sum(np.exp(normalized['avg_word_frequency']))
        normalized['softmax_pronounce_complexity'] = normalized['avg_pronounce_complexity']
        normalized['softmax_n_hyponyms'] = np.exp(normalized['n_hyponyms']) / np.sum(np.exp(normalized['n_hyponyms']))
        normalized['softmax_n_synonyms'] = np.exp(normalized['n_synonyms']) / np.sum(np.exp(normalized['n_synonyms']))
        normalized['softmax_n_senses'] = np.exp(normalized['avg_n_senses']) / np.sum(np.exp(normalized['avg_n_senses']))

        # Step 4: Combined metrics
        normalized['combined_metric_normalized'] = normalized.apply(self.__calculate_basicness_score, axis=1)
        normalized['combined_metric_softmax'] = normalized.apply(self.__calculate_basicness_score_softmax, axis=1)

        # Step 5: Min-max of combined metrics
        min_max_values = normalized.groupby('ili').agg(
        ...
        en_lemmas = df[df['Language'] == 'en'].groupby('ili')['Lemma'].apply(', '.join).reset_index(name='en_lemmas')
        it_lemmas = df[df['Language'] == 'it'].groupby('ili')['Lemma'].apply(', '.join).reset_index(name='it_lemmas')
        nb_lemmas = df[df['Language'] == 'nb'].groupby('ili')['Lemma'].apply(', '.join).reset_index(name='nb_lemmas')
        es_lemmas = df[df['Language'] == 'es'].groupby('ili')['Lemma'].apply(', '.join).reset_index(name='es_lemmas')

        # Merge the dataframes on 'ili' to get all language lemmas in a single dataframe
        lemmas_by_language = en_lemmas.merge(it_lemmas, on='ili', how='left') \
        ...
        example_en_lemma = (df[df['Language'] == 'en'].groupby('ili')['Lemma']
                            .apply(lambda x: x[x != 'None'].head(1)).reset_index(name='example_en_lemma'))
        example_en_gloss = (df[df['Language'] == 'en'].groupby('ili')['Gloss']
                            .apply(lambda x: x[x != 'None'].head(1)).reset_index(name='example_en_gloss'))
        ...
        # Step 9: Calculate basicness score by normalizing the combined metric
        result['basicness_score'] = (result['combined_metric_normalized'] - result['min_combined_normalized']) / \
                                    (result['max_combined_normalized'] - result[
                                        'min_combined_normalized'] + norm_basicness_denom)

        # Clipping the softmax input to prevent softmax overwhelming
        clipped_softmax_inputs = np.clip(result['combined_metric_softmax'], 0, 1)
        result['combined_metric_softmax_clip'] = clipped_softmax_inputs
        result['basicness_score_softmax_clip'] = np.exp(clipped_softmax_inputs) / np.sum(np.exp(clipped_softmax_inputs))

        # Scaling softax values, mean normalization
        mean_value = np.mean(result['combined_metric_softmax'])
        scaled_values = result['combined_metric_softmax'] - mean_value
        result['basicness_score_softmax_scale'] = np.exp(scaled_values) / np.sum(np.exp(scaled_values))

        # Plain softmax
        result['basicness_score_softmax'] = np.exp(result['combined_metric_softmax']) / np.sum(np.exp(result['combined_metric_softmax']))


        # Final selection of columns and sorting
        final_result = result[['Language', 'basicness_score',
                               'basicness_score_softmax',
                               'basicness_score_softmax_clip',
                               'basicness_score_softmax_scale',
                               'combined_metric_softmax',
                               'combined_metric_softmax_clip',
                               'word_in_children_res', 'word_in_second_lang_learn_res',
                               'en_lemmas', 'n_hyponyms', 'n_synonyms', 'n_syn_senses',
                               'avg_word_length', 'avg_pronounce_complexity',
                               'avg_word_frequency', 'avg_n_senses', 'normalized_length',
                               'normalized_pronounce_complexity',
                               'normalized_n_senses',
                               'normalized_frequency', 'normalized_n_hyponyms', 'normalized_n_synonyms',
                               'example_en_lemma', 'example_en_gloss', 'ili']]

        final_result = final_result.sort_values(by=['ili', 'combined_metric_softmax'], ascending=[True, False])

        self.__result_df = final_result

        # Add basicness rank
        ranked_df = self.add_basicness_rank(thresholds=thresholds)
    ...
    def evaluate_basicness_score_mse(self, use_test_set: bool = False) -> float:
        if self.__opt_set_df is None:
            raise ValueError("Ordinal regression loss requires an optimization dataset.")
        ...
        mse = np.mean((np.array(function_outputs) - np.array(human_outputs_transformed)) ** 2)
        ...
        for index, row in self.__result_df.iterrows():
            ...
            for i, row_annotation in self.__opt_set_df.iterrows():
                if row_annotation["ili"] == ili and row_annotation["Language"] == lang:
                    ...
                    if actual_binary_score == predicted_binary_score and predicted_binary_score == 1:
                        true_positives += 1
                    elif actual_binary_score == predicted_binary_score and predicted_binary_score == 0:
                        true_negatives += 1
                    elif actual_binary_score != predicted_binary_score and predicted_binary_score == 1:
                        false_positives += 1
                    elif actual_binary_score != predicted_binary_score and predicted_binary_score == 0:
                        false_negatives += 1
    ...
    def evaluate_accordance(self, opt_split: str = "full", allow_margin_error: bool = True,
                            thresholds: list = None) -> float:
        if self.__opt_set_df is None:
            raise ValueError("Ranking evaluation requires an optimization dataset.")
        ...
        for index, row in self.__result_df.iterrows():
            ...
            for i, row_annotation in function_result_df.iterrows():
                if row_annotation["ili"] == ili and row_annotation["Language"] == lang:
                    ...
                    elif allow_margin_error and abs(row_annotation["basicness_score"] - rank) == 1:
                        n_correct += 0.5
    ...
    def evaluate_ordinal_regression_loss(self, opt_split: str = "full", thresholds: list = [0.25, 0.5, 0.75]) -> float:
        """
    ...
    def evaluate_cumulative_ordinal_loss(self, opt_split: str = "full", thresholds: list = None,
        ...
        for index, row in self.__result_df.iterrows():
            ...
            for _, row_annotation in function_result_df.iterrows():
                if row_annotation["ili"] == ili and row_annotation["Language"] == lang:
                    ...
                    cumulative_probs = [1 - sigmoid((score - t) / gamma) for t in thresholds]
                    cumulative_probs = [0] + cumulative_probs + [1]  # Add bounds for P(y <= 0) = 0 and P(y <= 4) = 1

                    # Calculate probabilities for each rank
                    rank_probs = [cumulative_probs[i + 1] - cumulative_probs[i] for i in range(len(thresholds) + 1)]
    ...
    def scipy_objective(self, weights, metric, optimize_thresholds: bool = False, optimize_gamma: bool = False,
                        thresholds: list = None) -> float:
        basicness_weights = {
        ...
        if metric == 'accuracy':
            ...
            return self.evaluate_cumulative_ordinal_loss(thresholds=thresholds, gamma=gamma)
        else:
            raise ValueError("Invalid evaluation metric")

    def optimize_weights_diff_evo(self, word: str, metric: str = None,
                                  optimize_thresholds: bool = False,
                                  optimize_gamma: bool = False,
                                  thresholds = None) -> (dict, pd.DataFrame):
        if thresholds is None:
            thresholds = [0.25, 0.5, 0.75]
        ...
        objective_function = partial(self.scipy_objective, metric=metric, optimize_thresholds=optimize_thresholds,
                                     optimize_gamma=optimize_gamma, thresholds=thresholds)

        result = differential_evolution(objective_function, bounds, mutation=(0.5, 1.9), recombination=0.7,
                                        disp=True, maxiter=2000, workers=-1)
    ...
    def add_basicness_rank(self, thresholds: List[float] = None) -> pd.DataFrame:
        """
        Convert the basicness scores in the input dataframe to ranks, adding a new column 'basicness_rank' with the rank.
        Scores are mapped to the range from 1 to 4, where 4 is the highest basicness score and 1 the lowest.
        If the thresholds are provided, the scores are mapped according to them.
        Otherwise, each score is ranked within its ILI group across languages, with the highest score receiving rank 4.
        In this last case there will always be 4 ranks.
        :param thresholds: If provided, the thresholds defining the rank boundaries.
        """
        # Copy the input dataframe to avoid modifying the original data
        ranked_data = self.__result_df.copy()

        if thresholds is not None:
            # Map the rank to the range from 1 to 4, where N is the number of thresholds
            ranked_data['basicness_rank'] = ranked_data['basicness_score'].apply(
                lambda x: map_score_to_rank(x, thresholds)
            )
        else:
            # Group by 'ili' to rank scores within each unique ILI across languages
            ranked_data['basicness_rank'] = (ranked_data.groupby('ili')['basicness_score']
                                             .rank(method='dense', ascending=False)
                                             .astype(int))

            # Map the rank to the range from 1 to 4, where 4 is the highest basicness score and 1 the lowest
            ranked_data['basicness_rank'] = (ranked_data['basicness_rank']
                                             .map({1: 4, 2: 3, 3: 2, 4: 1})
                                             .fillna(1).astype(int)
                                             )

        # Reorder columns to place 'basicness_score' and 'basicness_rank' immediately after 'Language'
        columns_order = ['Language', 'basicness_score', 'basicness_rank'] + \
                        [col for col in ranked_data.columns if
                         col not in ['Language', 'basicness_score', 'basicness_rank']]
        ranked_data = ranked_data[columns_order]
    ...
    def __reset_df(self):
        self.__result_df = self.__input_df.copy()


def map_score_to_rank(basicness_score: float, thresholds: list = None) -> int:
    """
    Map a basicness score (0 to 1) to an ordinal rank based on thresholds.
    :param basicness_score: Predicted score (0 <= score <= 1).
    :param thresholds: List of thresholds defining the rank boundaries.
                       Default thresholds map [0-0.25, 0.25-0.5, 0.5-0.75, 0.75-1.0].
    :return: The rank (1 to N+1), where N is the number of thresholds.
    """
    if thresholds is None:
        thresholds = [0.25, 0.5, 0.75]

    # Iterate through the thresholds and assign the rank
    for rank, threshold in enumerate(thresholds, start=1):
        if basicness_score <= threshold:
            return rank

    # If the score exceeds the highest threshold, assign the highest rank
